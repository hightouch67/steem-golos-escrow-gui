...
var BLOCKCHAINS = {
	steem: {
		...
		agents:[{name:"future.witness",fee:0.001}]
	}},
	BLOCKCHAIN,
	transaction = {
		id: gup('id')
...
$(function() {
	...
	for (let index = 0; index < BLOCKCHAIN.agents.length; index++) {
		const agent = BLOCKCHAIN.agents[index];
		$('#sendAgent').append('<option value="' + agent.name + '" data-fee="' + parseFloat(agent.fee).toFixed(3) + '">' + agent.name +' - fee: ' + agent.fee + '</option>');
